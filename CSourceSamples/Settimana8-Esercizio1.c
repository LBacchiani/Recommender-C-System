...
static int numStudentiInseriti=0; //Scope a livello di file sorgente
...
int main(void) {
    ...
    studente arrayStudenti[MAX];
    int scelta=0;
    
    while (true) {
        scelta=menu();
        switch (scelta) { //Switch sulla scelta del menu
            case 1:
...
short int menu (void) { //Funzione che stampa a video il menu, Restituisce il numero inserito dall'utente.
    ...
    do {
        ...
        printf("\n\nSeleziona scelta successiva:\n\n1. Aggiungi studente;\n"
               ...
               "\n\nScelta (numero): ");
        scanf("%hi", &scelta);
    } while (scelta<1 || scelta>4); //Controllo input
    getchar();
    return scelta;
}
...
bool aggiungiStudente (studente* arrayStudenti) { //Aggiunta studente. Ritorna vero/falso (operazione riuscita o no).
    ...
    if (numStudentiInseriti==MAX) { //Se il vettore è pieno
...
bool modificaPianoDiStudi (studente* arrayStudenti) { //Modifica il piano degli studi per lo studente
    ...
    fgets(matricola, sizeof(matricola), stdin);
    for (int i=0; i<numStudentiInseriti; i++)  //Scorro ogni studente fino a trovare quello desiderato
        if (strcmp(arrayStudenti[i].nMatricola, matricola)==0) {
            ...
            for (int indiceInsegnamento=0; indiceInsegnamento<NUM_ESAMI; indiceInsegnamento++) {
                ...
                //Eliminazione carattere newline
          arrayStudenti[i].pianoDiStudi[indiceInsegnamento].nomeInsegnamento[strcspn(arrayStudenti[i].pianoDiStudi[indiceInsegnamento].nomeInsegnamento, "\n")] = 0;
